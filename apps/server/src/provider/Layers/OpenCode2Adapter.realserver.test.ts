...
function ensurePinnedCli(): { readonly binary: string } | { readonly reason: string } {
  ...
  const npmInstall = NodeChildProcess.spawnSync(
    "npm",
    [
      "install",
  ...
  const bunInstall = NodeChildProcess.spawnSync(
    "bun",
...
beforeAll(async () => {
  ...
  // oxlint-disable-next-line t3code/no-manual-effect-runtime-in-tests -- Server boot runs in a plain vitest beforeAll on purpose (see the harness note above); it.effect cannot host lifecycle hooks, so this one setup query runs the client manually.
...
// `it.live` (not `it.effect`): these tests await a real external server, so
...
describe("OpenCode 2 provider contract (real pinned server)", () => {
  it.live(
    ...
    (ctx) => {
      ...
      return Effect.gen(function* () {
        ...
        const inventory = yield* loadOpenCode2Inventory(client, live.workspaceDir);
  ...
  it.live(
    ...
    (ctx) => {
      ...
      if (!live) return ctx.skip();
      return Effect.gen(function* () {
        const snapshot = yield* checkOpenCode2ProviderStatus(testSettings(), live.workspaceDir);
  ...
  it.live(
    ...
    (ctx) => {
      ...
      return withAdapter(testSettings(), (adapter, collected) =>
        Effect.gen(function* () {
  ...
  it.live(
    ...
    (ctx) => {
      ...
      return Effect.gen(function* () {
        const firstRun = yield* withAdapter(testSettings(), (adapter, collected) =>
          Effect.gen(function* () {
        ...
        yield* withAdapter(testSettings(), (adapter, collected) =>
          Effect.gen(function* () {
            yield* adapter.startSession({
              threadId,
              runtimeMode: "full-access",
  ...
  it.live(
    ...
    (ctx) => {
      ...
      return Effect.gen(function* () {
        const outcome = yield* withAdapter(testSettings({ password: wrongPassword }), (adapter) =>
          Effect.gen(function* () {
  ...
  it.live(
    ...
    (ctx) => {
      ...
      return Effect.gen(function* () {
        const outcome = yield* withAdapter(testSettings(), (adapter) =>
          adapter
            .startSession({
              threadId,
              runtimeMode: "full-access",
