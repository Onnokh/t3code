...
import { SeoDataDate, SeoFreshnessBanner, SeoSiteSwipeSurface } from "./SeoUi";
...
export function SeoPageDetailScreen({ route }: StaticScreenProps<Params>) {
  ...
  return (
    <SeoSiteSwipeSurface sites={sites} selectedSiteId={siteId} onSelect={select}>
      <ScrollView
        ...
        {page ? (
          <>
            ...
            {page.actions.length === 0 ? (
              <Text className="text-sm text-foreground-muted">
                No Actions logged for this page.
              </Text>
            ) : (
