...
type ConnectionsNewRouteParams = {
  ...
  /**
   * The route to replace this screen with after a successful pairing, used
   * when this screen is the first screen of its stack and there is nothing
   * to go back to. The Devski pairing stack supplies no return route: the
   * root shell swaps the pairing stack for the tabs on its own once an
   * authorized environment exists.
   */
  ...
  const connectAndClose = useCallback(
    async (pairingUrl: string, replaceWithHome: boolean) => {
      ...
      try {
        ...
        if (replaceWithHome) {
          // Only the development deep link asks for this, and that link
          // always lands in the T3 stack, which owns a Home route.
          navigation.dispatch(StackActions.replace("Home"));
        } else if (navigation.canGoBack()) {
          navigation.goBack();
        } else if (returnRoute) {
          navigation.dispatch(StackActions.replace(returnRoute));
        }
        // The Devski pairing stack has one screen and no return route. It
        // needs no navigation here: the root shell replaces the pairing
        // stack with the tabs as soon as the environment is authorized.
      } finally {
        setIsSubmitting(false);
      }
    },
    [navigation, onChangeConnectionPairingUrl, onConnectPress, returnRoute],
  );
