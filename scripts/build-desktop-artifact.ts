...
const DESKTOP_APP_ID = "com.plowtools.plowcode";
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "PLOW Code (Nightly)"
    : (desktopPackageJson.productName ?? "PLOW Code");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "PLOW-Code-${version}-${arch}.${ext}",
    directories: {
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      target: [target],
      executableName: "plowcode",
      icon: "icons",
      category: "Development",
      desktop: {
        entry: {
          StartupWMClass: "plowcode",
        },
      },
    };
  }
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    description: "PLOW Code desktop build",
    author: "PLOW Tools",
